	...
	public MoverServiceRequest GenerateStage(Vector3 pos, Quaternion rotation, PickAndPlaceStage curStage, RobotArmTaskController controller)
	{
		...
		switch (curStage)
		{
			case PickAndPlaceStage.PreAction:
				...
				request.constraint = RobotArmController.GetOrientationConstraints(request.destination_pose.orientation, 180);
				request.scale_velocity = 0.07f;
				request.scale_acceleration = 0.05f;
				break;
			...
			case PickAndPlaceStage.MainAction:
				...
				request.scale_velocity = 0.01f;
				request.scale_acceleration = 0.001f;
				break;
